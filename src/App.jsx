    ...
    return (
      <React.Fragment key={t.id}>
        <tr className={isExpanded ? "tpc-row-expanded" : ""}>
          ...
          <td><span className={`tpc-group-pill tpc-group-pill-${t.group}`}>{groupLabel(t.group)}</span></td>
  ...
  return (
    <div className="tpc-app">
      <style>{`
        ...
        .tpc-landing { text-align: center; padding: 48px 16px 24px; overflow-y: auto; }
        .tpc-landing-sub { font-size: 13.5px; color: var(--muted); margin: 0 0 28px; }
        .tpc-landing-cards {
          display: flex; gap: 8px; max-width: 660px; margin: 0 auto; padding: 10px;
        ...
        .tpc-landing-card {
          ...
          text-align: left; cursor: pointer; transition: filter .12s;
          display: flex; flex-direction: column;
        }
        .tpc-landing-card:hover { filter: brightness(1.04); }
        .tpc-landing-card-header {
          font-family: 'Sora', sans-serif; font-size: 17px; font-weight: 700; color: #fff;
          padding: 18px 22px 8px; text-shadow: 0 1px 3px rgba(0,0,0,0.35);
        }
        .tpc-landing-card-body { font-size: 12px; padding: 2px 22px 18px; font-weight: 500; }
        ...
        .tpc-landing-card-CHUC { background: linear-gradient(180deg, #2E5AAC 0%, #EAF0FC 80%, #fff 100%); }
        .tpc-landing-card-TUNG { background: linear-gradient(180deg, #C2410C 0%, #FBEAE0 80%, #fff 100%); }
        .tpc-landing-card-TRUONG { background: linear-gradient(180deg, #0E7490 0%, #E1F1F5 80%, #fff 100%); }
        
        .tpc-stats {
          display: flex; gap: 8px; padding: 10px; margin-bottom: 14px; flex-shrink: 0;
        ...
        .tpc-stat-card:hover { filter: brightness(1.04); }
        .tpc-stat-card.active::before {
          content: ""; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: #fff;
        }
        .tpc-stat-header { padding: 10px 14px 6px; color: #fff; font-size: 12.5px; font-weight: 700; text-shadow: 0 1px 3px rgba(0,0,0,0.35); }
        ...
        .tpc-stat-total { background: linear-gradient(180deg, var(--gold) 0%, var(--gold-bg) 80%, #fff 100%); }
        .tpc-stat-total.active { background: linear-gradient(180deg, var(--gold-ink) 0%, var(--gold-bg) 80%, #fff 100%); }
        .tpc-stat-completed { background: linear-gradient(180deg, var(--green) 0%, var(--green-bg) 80%, #fff 100%); }
        .tpc-stat-completed.active { background: linear-gradient(180deg, #0E5C3F 0%, var(--green-bg) 80%, #fff 100%); }
        .tpc-stat-overdue { background: linear-gradient(180deg, var(--red) 0%, var(--red-bg) 80%, #fff 100%); }
        .tpc-stat-overdue.active { background: linear-gradient(180deg, #96271D 0%, var(--red-bg) 80%, #fff 100%); }

        .tpc-tabs {
          display: flex; gap: 8px; padding: 10px; margin-bottom: 20px; flex-shrink: 0;
          border: 1px solid var(--border); border-radius: 12px; background: var(--surface);
          box-shadow: 0 1px 2px rgba(16,24,40,0.04);
        }
        ...
        .tpc-tab:hover { filter: brightness(1.04); }
        .tpc-tab.active::before {
          content: ""; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: #fff;
        }
        .tpc-tab-header { padding: 10px 14px 6px; color: #fff; font-size: 13px; font-weight: 700; text-shadow: 0 1px 3px rgba(0,0,0,0.35); }
        .tpc-tab-body { padding: 2px 14px 12px; }
        .tpc-tab-ALL { background: linear-gradient(180deg, #8891A5 0%, #F1F2F5 80%, #fff 100%); }
        .tpc-tab-ALL.active { background: linear-gradient(180deg, var(--navy) 0%, var(--canvas) 80%, #fff 100%); }
        .tpc-tab-CHUC { background: linear-gradient(180deg, #90A8D6 0%, #EAF0FC 80%, #fff 100%); }
        .tpc-tab-CHUC.active { background: linear-gradient(180deg, #2E5AAC 0%, #EAF0FC 80%, #fff 100%); }
        .tpc-tab-TUNG { background: linear-gradient(180deg, #E0966E 0%, #FBEAE0 80%, #fff 100%); }
        .tpc-tab-TUNG.active { background: linear-gradient(180deg, #C2410C 0%, #FBEAE0 80%, #fff 100%); }
        .tpc-tab-TRUONG { background: linear-gradient(180deg, #74B7C6 0%, #E1F1F5 80%, #fff 100%); }
        .tpc-tab-TRUONG.active { background: linear-gradient(180deg, #0E7490 0%, #E1F1F5 80%, #fff 100%); }
        ...
        .tpc-btn-filter-toggle:hover { background: var(--canvas); }
        .tpc-filter-badge {
        ...
        .tpc-filter-group label { font-size: 12px; color: var(--muted); font-weight: 500; }
        ...
        .tpc-spacer { flex: 1; }
        .tpc-btn-secondary {
          background: #fff; border: 1px solid var(--border); color: var(--ink); border-radius: 8px;
        ...
        .tpc-table-header {
          flex-shrink: 0; background: var(--navy); color: #fff; font-size: 13px; font-weight: 700;
          padding: 11px 16px; display: flex; align-items: center; gap: 8px;
        }
        .tpc-table-header-count {
          background: rgba(255,255,255,0.18); font-size: 11.5px; font-weight: 700; padding: 1px 8px;
          border-radius: 999px;
        }
        .tpc-table-scroll { flex: 1 1 auto; min-height: 0; overflow: scroll; -webkit-overflow-scrolling: touch; }
        .tpc-table-scroll::-webkit-scrollbar { width: 10px; height: 10px; }
        .tpc-table-scroll::-webkit-scrollbar-track { background: #F3F5F9; }
        .tpc-table-scroll::-webkit-scrollbar-thumb { background: #C7CCD6; border-radius: 6px; }
        .tpc-table-scroll::-webkit-scrollbar-thumb:hover { background: #A9AFBC; }
        ...
        .tpc-form-row label { font-size: 12.5px; font-weight: 500; color: var(--muted); }
        .tpc-form-row .tpc-input, .tpc-form-row .tpc-select, .tpc-form-row textarea {
          ...
          border: 1px solid var(--border); border-radius: 7px; color: var(--ink); resize: vertical;
        }
        ...
        @media (max-width: 720px) {
          ...
          .tpc-stat-card { min-width: 0; }
          .tpc-stat-header { padding: 6px 8px; font-size: 9px; text-align: center; }
          .tpc-stat-body { padding: 7px 8px 9px; text-align: center; }
          .tpc-stat-value { font-size: 19px; }

          .tpc-tabs { margin-bottom: 12px; }
          .tpc-tab { min-width: 0; flex: 1 1 0; }
          .tpc-tab-header { padding: 6px 5px; font-size: 9px; text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
          .tpc-tab-body { padding: 6px 5px 7px; text-align: center; }
          .tpc-tab-count { font-size: 8.5px; white-space: nowrap; }
          .tpc-bar { margin-top: 6px; }
          ...
          .tpc-table-scroll { overflow: scroll; -webkit-overflow-scrolling: touch; }
          .tpc-table-header { padding: 9px 14px; font-size: 12px; }
          ...
          .tpc-landing-cards { flex-direction: column; max-width: 320px; }
          .tpc-landing-card { border-right: none; border-bottom: 1px solid rgba(16,24,40,0.08); }
          .tpc-landing-card:last-child { border-bottom: none; }
      ...
      <div className="tpc-body">
        ...
        <div className="tpc-stats">
          ...
          >
            <div className="tpc-stat-header">Công việc</div>
            <div className="tpc-stat-body">
          ...
          <button className={`tpc-stat-card tpc-stat-completed ${activeStatus === "completed" ? "active" : ""}`} onClick={() => setActiveStatus("completed")}>
            <div className="tpc-stat-header">Hoàn thành</div>
            <div className="tpc-stat-body">
          ...
          <button className={`tpc-stat-card tpc-stat-overdue ${activeStatus === "overdue" ? "active" : ""}`} onClick={() => setActiveStatus("overdue")}>
            <div className="tpc-stat-header">Quá hạn</div>
            <div className="tpc-stat-body">
        ...
        {isLockedViewer ? (
          ...
          <div className="tpc-tabs">
            <button className={`tpc-tab tpc-tab-ALL ${activeGroup === "ALL" ? "active" : ""}`} onClick={() => setActiveGroup("ALL")}>
              <div className="tpc-tab-header">Tất cả nhóm</div>
              <div className="tpc-tab-body">
            ...
            {GROUPS.map((g) => {
              ...
              return (
                <button key={g.key} className={`tpc-tab tpc-tab-${g.key} ${activeGroup === g.key ? "active" : ""}`} onClick={() => setActiveGroup(g.key)}>
                  <div className="tpc-tab-header">{g.label}</div>
                  <div className="tpc-tab-body">
                    <div className="tpc-tab-count">{w.o + w.p} công việc</div>
                    {w.total > 0 && (
                      <div className="tpc-bar">
                        <span style={{ width: `${(w.p / w.total) * 100}%`, background: "var(--gold)" }} />
        ...
        <div className="tpc-table-wrap">
          <div className="tpc-table-header">
            Danh sách công việc
            <span className="tpc-table-header-count">{incompleteTasks.length + completedTasks.length}</span>
          </div>
          <div className="tpc-table-scroll">
